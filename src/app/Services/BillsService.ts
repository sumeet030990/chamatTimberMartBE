 ...
 * @returns
 */
const formatBillData = (billData: any) => {
  const { customer_details } = billData;

  return {
    user_id: customer_details.user[0].value,
    user_object: customer_details.user,
...
const storeBill = async (savedUser: any, validationData: Prisma.billsCreateInput) => {
  const formattedBillData = formatBillData(validationData);
 ...
 * @returns
 */
const updateBill = async (id: string, data: object) => {
  return BillsRepository.updateBill(id, data);
};
